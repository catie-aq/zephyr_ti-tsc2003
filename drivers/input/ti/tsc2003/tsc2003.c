...
static int tsc2003_process(const struct device *dev) {
    ...
    ret = tsc2003_read_register(dev, CMD_MEASURE_X, &x);
    if (ret < 0) {
        return ret;
    }

    ret = tsc2003_read_register(dev, CMD_MEASURE_Y, &y);
    if (ret < 0) {
        return ret;
    }
...
static void tsc2003_work_handler(struct k_work *work) {
    ...
    struct tsc2003_data *data = CONTAINER_OF(dwork, struct tsc2003_data, work);
...
static void tsc2003_isr_handler(const struct device *dev, struct gpio_callback *cb, uint32_t pins) {
    ...
    k_work_reschedule(&data->work, K_MSEC(DEBOUNCE_INTERVAL_MS)); 
}
...
static int tsc2003_init(const struct device *dev) {
    ...
    k_work_init_delayable(&data->work, tsc2003_work_handler);  
    ...
    /* Initial touch read to validate connection */
    uint16_t value;
    r = tsc2003_read_register(dev, CMD_MEASURE_X, &value);
    if (r < 0) {
        return r;
    }
    LOG_INF("Initial touch read: X=[%d]", value);
